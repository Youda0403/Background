...
| **Zine** | Photocopied record sleeve: heavy grain, halftone plate, barcode and numeral rails, rotated date, and the riso second plate — the accent — carrying the rule at the title, the bracket corners and the release line. The rule sits **on the first line's baseline and is drawn before the words**, so the type prints over it and the script's descenders sweep through it. Struck across the lockup at 24% of its height, over the top of the letters, it was a line through a mistake; on the baseline, underneath, it is a line the words are written on. `poster.headline` returns `firstBaseline` for exactly this, in the measure pass as well, so a caller can hang a rule on the type instead of guessing a fraction of the lockup's height. |
| **Aura** | Colour blooms and a soft photo window. The gentle one. **The marks are stratified.** Uniform random placement is uniform on average and lumpy in any single draw: six marks all landed in the top half of a phone page and the bottom third had nothing on it. `deco.points` can now divide the page into as many horizontal bands as there are marks and fill them in a *shuffled* order — shuffled because filling top to bottom would be a different bias for the same price, the first mark being both the hero and the biggest. The bands overlap by half a band so a mark whose own band is under a keep-out zone still has somewhere to go. The marks are also half again as big, since at 14–30 per mille nobody would have noticed if they had been better spread; and the blooms are wider and sit lower, because the glow and the photo window are both in the upper half and blooms peaking at a quarter of the way down piled colour where there was already plenty. The window has no outline: an edge was there to stop the photograph dissolving into the page, but a ring drawn round a feathered window is the one hard line in the softest layout in the set and it read as a sticker rather than as a window. The feather is the edge now that it ramps smoothly. A wide canvas gives the photograph and the whole type group one shared centreline — the glow itself is anchored there too, to the left column's centre, rather than to wherever a photo happened to be placed. Anchoring it to the plate meant that with no photo loaded the glow fell back to the full canvas centre, drifting into the gutter between the two columns and lining up with neither the empty photo slot nor the text. |
