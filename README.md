...
| **Aura** | Colour blooms and a soft photo window. The gentle one. **The aura is the layout.** It was sized at half the photo window — a bloom tucked behind a circle, which on a phone page is a small warm patch in a large field of flat colour — and the scattered marks were quietly doing the work of filling the rest. So the marks start at zero here and the glow carries the page: it reaches past the window on every side, and with no photograph it spans the better part of the short edge, which turns the empty state into the best-looking page in the set rather than the emptiest. **The accent moved to the rails.** It used to ride on the scattered marks, and with those off it reached the page nowhere: the glow's accent core is covered by the photograph, and what escapes past the edge is the tail of a soft blend — 0.004% of the page on the muted palettes, which is another way of saying the swatch on the palette card showed a colour the wallpaper did not contain. A soft blend also cannot hold its hue: at three-quarters opacity a mint over a coral page measures 32° away from the mint, because the quarter that is page pulls it. The two foot rails are set in the accent at full opacity instead, where the glyph interiors are exactly the colour the card promises. The blooms behind it are wider and sit lower, because the glow and the window are both in the upper half and blooms peaking a quarter of the way down piled colour where there was already plenty. When marks ARE asked for, they are **stratified**: uniform random placement is uniform on average and lumpy in any single draw — six marks all landed in the top half of a phone page — so `deco.points` divides the page into as many horizontal bands as there are marks and fills them in a *shuffled* order, shuffled because filling top to bottom would be a different bias for the same price, the first mark being both the hero and the biggest. The bands overlap by half a band so a mark whose own band is under a keep-out zone still has somewhere to go. The window has no outline: an edge was there to stop the photograph dissolving into the page, but a ring drawn round a feathered window is the one hard line in the softest layout in the set and it read as a sticker rather than as a window. The feather is the edge now that it ramps smoothly. A wide canvas gives the photograph and the whole type group one shared centreline — the glow itself is anchored there too, to the left column's centre, rather than to wherever a photo happened to be placed. Anchoring it to the plate meant that with no photo loaded the glow fell back to the full canvas centre, drifting into the gutter between the two columns and lining up with neither the empty photo slot nor the text. |
