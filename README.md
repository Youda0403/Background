...
appears on the layouts that actually scatter motifs; on the others the
sliders moved and the page did not change, which reads as a broken control
rather than as one that does not apply. Where the group does appear, the
count may be zero, and on Column and Aura it starts there: decoration is an
option the page offers, not a tax it charges.

The same is now true field by field in the type group. **Each layout
declares which type controls its drawing code actually reads** — Grid sets
every letter, crossword and rail alike, in the body face, so the display
fonts and the headline size never reach it; Portal and Column size their
display type against a shape rather than a scale; only Aura offers a choice
of how two lines are set, because it is the only layout that asks the state
what that choice is. Six of the seven dials were being shown to every
layout, and most of them did nothing on most of them.

**A way back from the fiddling.** 되돌리기 puts every dial back where the
layout and the palette left it and does not touch a thing you chose or
wrote — the layout, the palette, the words, the device, the photo and its
framing all stay. The button also says whether there is anything to undo,
rather than being a control that may or may not do something. It surfaced a
real bug when it first ran: a first visit built the state from the global
fallbacks and named a layout without applying that layout's own defaults,
so the very first page anybody saw set Portal's display line in Cormorant
over a washed photo when Portal asks for a Didone and a duotone. Share
links were always fine — the serialiser records the effective state, not a
delta from the layout — it was only ever the empty case. The *Discretion*
...
| **Zine** | Photocopied record sleeve: heavy grain, halftone plate, barcode and numeral rails, rotated date, and the riso second plate — the accent — carrying the bracket corners and the release line. **There is no rule at the title.** It was struck across the lockup at 24% of its height, over the top of the letters, which reads as a line through a mistake; moved to the first line's baseline and drawn underneath the words it read better, and still read as a line at a script's baseline. The second plate does not need it. |
| **Aura** | A poster page: a big Didone title flush left, **three numbered photo windows** — an oval, a rounded card and an octagon — stepping down alternate sides, a folded gradient ribbon curling through them, spiky stars, a year badge, a column of crosses in the margin and a barcode at the foot. It replaces a layout that was a circle parked above a centred stack, where the photograph was the last thing the design thought about: one small window with a glow behind it, identical on every page. Here the picture appears **three times, in three shapes, at three crops**, and the composition is built round those windows. Everything is placed in fractions of the STAGE — the measure by the keep-out band — so the whole page scales together instead of drifting apart on a tablet, and the boxes are laid so no two of them share a rectangle: the frames alternate sides and the small type lives in the gaps beside them, never positioned relative to how long a string happens to be. A wide canvas gets its own table, the title taking the left half and the windows standing in the right, because a stepped column of frames on a 16:9 page is three stamps in a very long field. **The windows are three sizes**, the largest three and a half times the smallest: at near enough the same area they read as a set of stamps rather than as a composition. **The ribbon took four tries.** A smooth polygon whose width breathes along its length draws a worm — a tube with no surface. Ribbon reads through creases, where the band pinches to a hairline, and through a back face, the duller side you see between one crease and the next; so the half-width is a full sine that really does close at every half turn, and each run between two creases is filled as its own quad, alternating front and back. It also has to be LONG — a short one is a stub, and a stub with folds in it is a caterpillar — so it enters off the top of the page, sweeps over the head rule and comes down behind the display line, which is drawn after it. Its two ends are pastel mixes of the accent and the duotone: an accent-to-ink gradient goes through mud on any warm palette, and on a deep page the same mix lands as slate twice over — the duotone is already dark, and the shaded face was mixing toward a near-white ink, which flattened the crease away — so a dark ground pulls the second stop toward the accent to hold a hue and shades toward the ground rather than toward the ink. (It also has to be `mixHex` rather than `mix` at the call site — the streamer mixes those colours again internally, and `mix` returns an `rgb()` string the next mix cannot parse, which is exactly how the first cut came out the colour of a slug.) |
| **Column** | A full-bleed photo field with a **receipt** laid on it: torn top and bottom edges, a shop line, the pair name as the ticket's name, the two names and the tags as line items with dotted leaders, a total in the palette's colour, the caption as the memo, and a barcode over the footnote. It was a dictionary column — a plain rectangle of paper with a headword and an etymology — and the idea was right but a rectangle full of paragraphs is a *panel*, not an object, so it leant on scattered motifs to stop the rest of the page reading as empty. A slip of paper with a torn edge and a barcode is a **thing**, and things need nothing sprinkled around them: the motif count starts at zero here. What balances a piece of paper is **more paper** — the open half carries a note, a clipped slip and a **ticket** — three different objects, not one object three times — laid at fixed fractions of the field rather than scattered, because the eye reads three sheets at three angles as a desk and the same three at random as a mess. Their ruled lines are lines, not text: a second block of readable words over there would compete with the ticket for the eye. The ticket at the foot carries a perforation and a stub, and on the stub the only piece of colour on this side of the page, which is what stops the palette living entirely in the right-hand column. The paperclip is one continuous path — a free end, the outer legs, the bottom turn, a wider turn over the top that brings the wire back inside, and a second free end — because drawn as two strokes it read as a bracket and a staple lying on top of each other. A rubber stamp was tried there first and read as a novelty. The whole ticket is measured before anything is drawn — it is exactly as tall as its own contents, and if it cannot fit the band it is set one notch smaller and measured again rather than being cut, because the contents are clipped to the slip's outline and a clipped line is still a drawn line. The paper is a *card*: on a dark palette it stays light, where painting it in the page colour put a navy box on a navy field and lost it entirely. On a **3:1 header** two things change: the per-mille unit comes off a 500px height, so the cap would make the ticket 14% of the width and turn the hero of the page into a stamp — there the band's own height sets its width instead — and a desk three times wider than the ticket gets two more sheets, because three spread across a header read as three things adrift while five read as a desk. |
