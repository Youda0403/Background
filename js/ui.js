...
(function (W) {
  ...
  var HELP = {
    ...
    headlineStyle: '메인 문구가 두 줄일 때, 두 줄을 어떻게 짤지 골라요.',
    headlineScale: '메인 문구 크기. 키워도 여백은 레이아웃이 맞춰줘요.',
    microScale: '캡션·날짜·태그 같은 작은 글자 크기.',
  ...
  /* A `when` that asks the layout whether a type control reaches it. */
  function typeHas(key) {
    return function (s) {
      var L = (W.layoutRegistry || []).filter(function (x) { return x.id === s.layout; })[0];
      return !!(L && L.type && L.type.indexOf(key) >= 0);
    };
  }
  ...
  function spec() {
    return [
      ...
      },
      {
        ...
        items: [
          /* Each layout declares which of these its drawing code actually
             reads. Grid sets every letter in the body face; Portal and
             Column size their display type against a shape rather than a
             scale; only Aura offers a choice of how two lines are set.
             Showing the rest was showing controls that do nothing. */
          ...
          { t: 'slider', key: 'microScale', label: '작은 글자 크기', min: 0.7, max: 1.6, step: 0.01, help: HELP.microScale, when: typeHas('microScale') }
        ]
      },
      {
        ...
        items: [
          ...
          { t: 'note', text: '아래는 지금 고른 레이아웃에만 있는 옵션이에요.', when: function (s) { return s.layout === 'aura'; } },
      ...
      {
        /* Undo the fiddling, keep the design. Everything above the
           "다듬기" sections is something the user chose or wrote; the
           dials inside them are nudges, and there was no way back from a
           session of nudging short of starting over. */
        title: '되돌리기', hint: '다듬은 값만 기본으로',
        items: [
          { t: 'note', text: '레이아웃 · 팔레트 · 문구 · 사진은 그대로 두고, 폰트와 크기, 장식, 질감, 사진 보정처럼 직접 건드린 값만 이 레이아웃의 기본값으로 돌려놔요.' },
          {
            t: 'custom',
            render: function () {
              var el = W.controls.el;
              var wrap = el('div', 'field');
              var b = el('button', 'btn sm', '다듬기 초기화');
              b.type = 'button';
              b.style.alignSelf = 'flex-start';
              b.addEventListener('click', function () {
                st = S.resetTuning(st);
                panel.refresh();
                draw();
                toast('다듬기를 이 레이아웃의 기본값으로 되돌렸어요.');
              });
              wrap.appendChild(b);
              wrap._b = b;
              return wrap;
            },
            /* The button says whether there is anything to undo, rather
               than being a control that may or may not do something. */
            update: function (s, node) {
              var tuned = S.isTuned(s);
              node._b.disabled = !tuned;
              node._b.textContent = tuned ? '다듬기 초기화' : '이미 기본값이에요';
            }
          }
        ]
      }
  ...
  function initialState() {
    ...
    /* A first visit gets the DEFAULT LAYOUT'S OWN defaults, not the bare
       state object. `create()` names a layout but carries the global
       fallbacks — so the very first page anybody saw set Portal's display
       line in Cormorant with a washed photo, when Portal asks for a Didone
       and a duotone. Every share link is fine either way, because the
       serialiser records the effective state rather than a delta from the
       layout; it was only ever the empty case that came out wrong. */
    var fresh = S.create();
    S.applyLayoutDefaults(fresh, fresh.layout);
    return fresh;
  }
